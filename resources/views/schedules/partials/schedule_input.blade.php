                ...
                <table class="min-w-full border-separate border-spacing-0 text-sm">
                    <thead class="bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 sticky top-0 z-10 shadow">
                        <tr>
                            ...
                            <th class="px-4 py-3 border border-gray-200 dark:border-gray-700 text-left text-sm">Room</th>
                    ...
                    <tbody> 
                        ...
                        @foreach ($rooms as $room)
                            ...
                            @if ($groupedByTeacherAndDate->isEmpty())
                                <tr>
                                    <td class="px-4 py-3 border text-sm text-gray-500 dark:text-gray-300"> 
                                        <select name="teacher_id" class="teacher-select block w-full appearance-none text-xs font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl p-3 pr-10 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" data-room-id="{{ $room->id }}">
                                    ...
                                    <td class="px-4 py-3 border font-bold text-gray-800 dark:text-gray-200">{{ $room->roomname }}</td>
                                    @foreach ($timeSlots as $time => $slotKey)
                                        <td class="px-1 py-2 border text-center text-xs text-gray-500 dark:text-gray-300">
                                            <form class="schedule-form" data-room-id="{{ $room->id }}" data-time-slot="{{ $time }}" data-slot-key="{{ $slotKey }}">
                                                ...
                                                <select name="student_id" class="block w-full text-xs py-1 px-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 mb-1" required>
                                                ...
                                                <select name="subject_id" class="block w-full text-xs py-1 px-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            @else
                                @foreach ($groupedByTeacherAndDate as $group)
                                    <tr class="hover:bg-slate-50 dark:hover:bg-gray-800 align-top transition text-xs">
                                        <td class="px-4 py-3 border font-medium text-sm text-center text-gray-800 dark:text-gray-100">
                                            {{ $group->first()->teacher->name ?? 'N/A' }}
                                        </td>
                                        <td class="px-4 py-3 border font-bold text-gray-800 dark:text-gray-100">{{ $room->roomname }}</td>

                                        @foreach ($timeSlots as $time => $slotKey)
                                            @php
                                                $scheduledStudents = $group->filter(fn($schedule) => $schedule->{$slotKey});
                                            @endphp
                                            <td class="px-1 py-2 border align-top text-gray-800 dark:text-gray-200"> 
                                                @if($scheduledStudents->isNotEmpty())
                                                    @foreach($scheduledStudents as $schedule)
                                                    <div class="schedule-item bg-gray-100 dark:bg-gray-700 border rounded-md p-1 mb-1">
                                                        <div class="text-xs text-gray-600 dark:text-gray-200">{{ $schedule->student->name ?? 'N/A' }}</div>
