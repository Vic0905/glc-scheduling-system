...
<div id="createTeacherModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden opacity-0 transition-opacity duration-300">
    <div class="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-2xl mx-4 transform scale-95 transition-all duration-300">
        <!-- Header -->
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">Add Teacher</h2>
        ...
        <form action="{{ route('teachers.store') }}" method="POST" class="space-y-6">
            ...
            <!-- Nickname -->
            <div>
                <label for="createNickname" class="block text-sm font-medium text-gray-700 mb-1">Nickname</label>
            ...
            <div class="flex justify-end gap-3 pt-4">
                <button type="button" onclick="closeCreateModal()"
                    class="bg-gray-900 hover:bg-transparent px-6 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-gray-200 hover:border-gray-200 text-gray-100 hover:text-gray-900 rounded-xl transition ease-in duration-100">
                    Cancel
                </button>
                <button type="submit"
                    class="bg-blue-500 hover:bg-transparent px-6 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-blue-500 hover:border-gray-100 text-white hover:text-blue-900 rounded-xl transition ease-in duration-100">
                    Add Teacher
                </button>
            </div>
        </form>
